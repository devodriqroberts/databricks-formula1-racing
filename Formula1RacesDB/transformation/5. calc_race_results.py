...
spark.sql(f"""
          CREATE TABLE IF NOT EXISTS f1_presentation.calaculated_race_results
          (
            race_year INT,
            team_name STRING,
            driver_id INT,
...
spark.sql(f""" 
    CREATE OR REPLACE TEMP VIEW race_results_updates
    AS
    SELECT 
        races.race_year,
...
spark.sql("""
        MERGE INTO f1_presentation.calaculated_race_results tgt
        USING race_results_updates upd
